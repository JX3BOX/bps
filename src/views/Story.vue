<template>
    <div class="v-story">
        <el-tabs v-model="active" type="card" @tab-click="loadData">
        ...
        <div class="m-bps-story">
            ...
            <div class="u-history" v-if="active == 'history'">
                <div class="u-item" v-for="(history, i) in data" :key="i">
                    <h2 class="u-title">{{ history.name }}</h2>
                    <p v-for="(p, j) in history.descs" :key="j">
                        {{ p && p.desc }}
                    </p>
                </div>
            </div>
            <div class="u-person" v-if="active == 'person'">
                <div class="u-item" v-for="(person, i) in data" :key="i">
                    <h2 class="u-title">
                        ...
                        <span class="u-desc" :class="'v' + person.level">
                            {{ person.nameDesc || person.jobDesc }}
                        </span>
                        <span class="u-age">{{ person.age }}</span>
                    </h2>

                    <p v-for="(p, j) in person.descs" :key="j">
                        {{ p && p.desc }}
                    </p>
                </div>
            </div>
